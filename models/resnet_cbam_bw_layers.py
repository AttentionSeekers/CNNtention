...
class ResnetCBAM(nn.Module):
    def __init__(
        ...
        self.layer1 = self._make_layer(block, 16, layers[0], stride=1, reduction=reduction)
        #self.att1 = CBAM(c_in=16, reduction=reduction, kernel_size=7)
        self.layer2 = self._make_layer(block, 32, layers[1], stride=2, reduction=reduction)
        #self.att2 = CBAM(c_in=32, reduction=reduction, kernel_size=7)
        self.layer3 = self._make_layer(block, 64, layers[2], stride=2, reduction=reduction)
    ...
    def _make_layer(
            self,
            block: Type[Union[BasicBlock, Bottleneck, CBAM]],
            planes: int,
            blocks: int,
            stride: int = 1,
            reduction: int = 16
        ...
        for _ in range(1, blocks):
            ...
            layers.append(CBAM(c_in=planes, reduction=reduction, kernel_size=7))
